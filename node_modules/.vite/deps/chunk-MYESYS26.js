...
function useApplyDefaultValuesToTimeFieldInternalProps(internalProps) {
  ...
  const ampm = React34.useMemo(() => internalProps.ampm ?? adapter.is12HourCycleInCurrentLocale(), [internalProps.ampm, adapter]);
  return React34.useMemo(() => _extends({}, internalProps, validationProps, {
...
function useMeridiemMode(date, ampm, onChange, selectionState) {
  const adapter = usePickerAdapter();
  const cleanDate = React60.useMemo(() => !adapter.isValid(date) ? null : date, [adapter, date]);
  const meridiemMode = getMeridiem(cleanDate, adapter);
  const handleMeridiemChange = React60.useCallback((mode) => {
...
//# sourceMappingURL=chunk-MYESYS26.js.map
